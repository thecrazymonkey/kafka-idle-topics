...
- It has no active production during a configurable idle window (default 60 minutes, set via `-idleMinutes`).
- It has no data within its partitions.

Idle detection state is persisted to a JSON file (`idle-state.json` by default) so that the idle window survives restarts.
...
Usage of ./kafka-idle-topics:
  ...
  -idleMinutes int
    	Minutes of inactivity before a topic is a deletion candidate. Governs both production and consumption checks uniformly. Can be set using env variable KAFKA_IDLE_MINUTES (default 60)
  ...
  -stateFile string
    	Path to the persistent idle-state file. Mount a persistent volume at this path in containerised deployments. (default "idle-state.json")
  -username string
...
docker run \
  ...
  thecrazymonkey/kafka-idle-topics:latest
----

To persist the idle detection state across container restarts, mount a local directory and set the `-stateFile` flag:
...
docker run \
  ...
  thecrazymonkey/kafka-idle-topics:latest \
  -kafkaSecurity plain_tls \
  -stateFile /data/idle-state.json
----

NOTE: Without a mounted volume, the idle state is lost when the container stops, and the idle timer resets on every run.
...
[source,bash]
----
go get github.com/thecrazymonkey/kafka-idle-topics/cmd/kafka-idle-topics
go install github.com/thecrazymonkey/kafka-idle-topics/cmd/kafka-idle-topics
...
[source,bash]
----
git clone https://github.com/thecrazymonkey/kafka-idle-topics
